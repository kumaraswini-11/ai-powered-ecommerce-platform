...
import { StackedProductImages } from "@/components/order/stacked-product-images";
...
export const metadata = {
  title: "Your Orders | Ecommerce Shop",
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="space-y-4">
        {orders.map((order) => {
          ...
          const status = getOrderStatus(rawStatus);
          const StatusIcon = status.icon;

          const images = (itemImages ?? []).filter((url): url is string =>
            Boolean(url),
          );

          const visibleItemNames =
            itemNames?.slice(0, 2).filter(Boolean).join(", ") ?? "";

          return (
            <Link
              key={_id}
              href={`/orders/${_id}`}
              aria-label={`View order ${formatOrderNumber(orderNumber)}`}
