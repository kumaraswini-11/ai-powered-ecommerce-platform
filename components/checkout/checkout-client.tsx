...
import { useCartStock } from "@/components/checkout/use-cart-stock";
...
function EmptyCart() {
  return (
    <div className="mx-auto max-w-2xl px-4 py-16 text-center sm:px-6 lg:px-8">
      <ShoppingBag className="mx-auto size-16 text-zinc-300 dark:text-zinc-600" />
...
interface CartItemsSectionProps {
  ...
  totalItems: ReturnType<typeof useTotalItems>;
  stockMap: Map<
    string,
    { isOutOfStock: boolean; exceedsStock: boolean; currentStock: number }
  >;
        ...
        <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
          {items.map((item) => {
            ...
            return (
              ...
              >
                {/* Cart Item Image */}
                <div className="relative size-20 shrink-0 overflow-hidden rounded-md bg-zinc-100 dark:bg-zinc-800">
                  {item.image ? (
                    <Image
                      src={item.image}
                      alt={item.name}
...
// Renders the checkout page client-side, including cart items, stock checks, and payment summary.
// Handles empty cart, stock issues, and loading states.
